  ...
  useEffect(() => {
    ...
    async function fetchUsuario() {
      ...
      if (user) {
        ...
        if (data && data.nombre) {
          // Extraemos solo el primer nombre por estética (ej. "César" en vez de "César Alejandro")
          const primerNombre = data.nombre.split(' ')[0];
          setNombreUsuario(primerNombre);
        } else {
          // Plan B: Si no tiene perfil aún, usamos la primera parte de su correo
          setNombreUsuario(user.email.split('@')[0]);
        }
      }
    }

    // 2. Buscamos las ubicaciones (El código que ya tenías)
    async function fetchUbicaciones() {
      const { data, error } = await supabase.from('equipos').select('ubicacion');
      if (data) {
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#F8F9FA] text-slate-900 font-sans pb-10 sm:border-x sm:border-slate-200">
      ...
      <div className="px-6 mt-6">
        <div className="flex items-center text-slate-900 font-extrabold mb-5 gap-2">
